...
function Cube(props) {
  ...
  const [composer, savePass] = useMemo(() => {
    ...
    const chromaticAberrationPass = new EffectPass(targetCamera, CHROMATIC_ABERRATION)
    const targetEffectPass = new EffectPass(targetCamera, VIGNETTE)
    const effectPass = new EffectPass(camera, SMAA, BLOOM, DEPTH_OF_FIELD, VIGNETTE)
      ...
      {ARGS.map((args, index) => (
        <group key={`0${index}`} {...args}>
          ...
          <Plane args={[8, 6]} position={[-1.4, 0, -1]}>
            <meshStandardMaterial attach="material" map={savePass.renderTarget.texture} />
          </Plane>
        </group>
      ))}
    </>
...
function Planes(props) {
  const [aoMap, map, displacementMap, normalMap, roughnessMap] = useLoader(THREE.TextureLoader, [
  ...
  return (
    <group {...props}>
      <Plane args={[200, 200]} receiveShadow>
        <meshPhysicalMaterial {...materialProps} color={COLOR[COLOR_INDEX][2]} />
      </Plane>
      <Plane args={[200, 200]} rotation={[0, Math.PI / 2, 0]} position={[-40, 0, 0]} receiveShadow>
        <meshPhysicalMaterial {...materialProps} color={COLOR[COLOR_INDEX][2]} />
      </Plane>
      <Plane args={[200, 200]} rotation={[-Math.PI / 2, 0, 0]} receiveShadow>
        <meshPhysicalMaterial {...materialProps} color={COLOR[COLOR_INDEX][4]} />
      </Plane>
    </group>
  )
}
...
ReactDOM.render(
  <Canvas
    ...
    <pointLight position={[10, 1, 20]} intensity={0.7} castShadow shadow-mapSize-width={1024} shadow-mapSize-height={1024} />
